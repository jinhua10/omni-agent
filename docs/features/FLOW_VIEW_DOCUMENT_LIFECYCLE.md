# 📋 流程视图文档生命周期管理

> **实现时间**: 2025年12月26日  
> **功能**: 文档在流程视图中的完整生命周期管理  
> **状态**: ✅ 已完成

---

## 🎯 功能概述

实现了文档在处理流程视图中的完整生命周期管理，包括：

1. **处理完成后自动移除** - 索引存储完成后，文档从流程视图中自动消失
2. **重新加入流程** - 已完成的文档可以从浏览器视图重新加入到流程视图
3. **虚拟路径支持** - 基于存储服务的虚拟路径系统，支持多种存储后端
4. **实时进度监控** ⭐ - 通过 WebSocket 实时显示处理进度和状态更新

---

## 🔄 文档生命周期

### 阶段1: 上传与初始处理

```
用户上传文档
    ↓
保存到监听目录 (data/documents)
    ↓
触发自动处理流程
    ↓
显示在流程视图的待处理列表
```

### 阶段2: 处理流程

```
UPLOAD (0%) → 文档上传
    ↓
EXTRACT (20%) → 文本提取
    ↓
CHUNK (40%) → 智能分块
    ↓
VECTORIZE (60%) → 向量化
    ↓
INDEX (80%) → 索引存储
    ↓
COMPLETED (100%) → 处理完成
```

### 阶段3: 完成与归档

```
处理完成 (COMPLETED)
    ↓
文档从 data/documents 移动到 data/storage/documents
    ↓
从流程视图的待处理列表中移除 ⭐
    ↓
可在浏览器视图中查看
```

### 阶段4: 重新处理（可选）

```
在浏览器视图中找到文档
    ↓
点击"加入流程视图"按钮 ⭐
    ↓
从 storage 读取文档数据（虚拟路径）
    ↓
写入到 data/documents 临时目录
    ↓
从 storage 中删除原文档 ⭐ (避免索引冲突)
    ↓
重新显示在流程视图中
    ↓
可以重新配置并处理
```

---

## 🛠️ 实现细节

### 1. 前端 - 实时进度监控 ⭐

**文件**: `UI/src/components/rag-flow/DocumentProcessingFlow.jsx`

#### 功能1: 点击"开始处理"后自动监控

```javascript
// 开始处理文档
const startProcessDocument = useCallback(async (docId) => {
    const result = await ragStrategyApi.startProcessing(docId);
    if (result.success) {
        message.success('开始处理文档：' + docId);
        
        // ⭐ 自动选中该文档，显示处理进度
        setSelectedDocId(docId);
        
        // ⭐ 初始化进度状态
        setProgress({
            documentId: docId,
            documentName: docId,
            stage: 'UPLOAD',
            status: 'PROCESSING',
            percentage: 0,
            message: '开始处理...',
            startTime: Date.now()
        });
        
        loadDocumentsList();
    }
}, [message, loadDocumentsList]);
```

#### 功能2: WebSocket 实时更新

```javascript
// 建立 WebSocket 连接
useEffect(() => {
    if (!selectedDocId || demoMode) return;
    
    const client = new WebSocketClient('ws://localhost:8080/ws/progress');
    
    client.on('open', () => {
        console.log('✅ WebSocket 连接已建立');
        client.subscribe(selectedDocId); // 订阅文档进度
    });
    
    client.on('message', handleMessage); // 接收进度更新
    
    client.connect();
    
    return () => {
        client.close();
    };
}, [selectedDocId, demoMode]);
```

#### 功能3: 备用轮询机制

```javascript
// 轮询检查文档状态（防止 WebSocket 失败）
const pollInterval = setInterval(async () => {
    const response = await fetch(`/api/system/rag-config/document/${selectedDocId}`);
    const result = await response.json();
    if (result.success && result.data) {
        // 更新状态
    }
}, 5000); // 每 5 秒轮询一次
```

**效果**:
- ✅ 点击"开始处理"后立即显示进度条
- ✅ WebSocket 实时推送处理进度（0%→20%→40%→60%→80%→100%）
- ✅ 显示当前处理阶段（上传→提取→分块→向量化→索引→完成）
- ✅ 备用轮询机制确保进度更新可靠

---

### 2. 前端 - 流程视图自动刷新

**文件**: `UI/src/components/rag-flow/DocumentProcessingFlow.jsx`

#### 修改点1: 过滤已完成文档

```javascript
// 加载文档列表 - 只显示未完成的文档
const loadDocumentsList = useCallback(async () => {
    setLoading(true);
    try {
        const response = await fetch('/api/system/rag-config/documents-status');
        const result = await response.json();
        if (result.success) {
            // ⭐ 只显示未完成的文档（排除COMPLETED状态）
            const docs = Object.values(result.data).filter(doc => doc.status !== 'COMPLETED');
            setDocumentsList(docs);
            console.log('📋 加载文档列表:', docs.length, '个待处理文档');
        }
    } catch (error) {
        console.error('加载文档列表失败:', error);
    } finally {
        setLoading(false);
    }
}, []);
```

#### 修改点2: 处理完成时自动刷新

```javascript
// 处理 WebSocket 消息
const handleMessage = useCallback((message) => {
    if (message.type === 'progress') {
        setProgress(message.data);

        // ⭐ 如果完成，刷新文档列表（移除已完成文档）
        if (message.data.status === 'COMPLETED') {
            console.log('✅ 文档处理完成，刷新列表移除该文档:', message.data.documentId);
            // 延迟刷新，确保后端状态已更新
            setTimeout(() => {
                loadDocumentsList();
            }, 1000);
            
            if (onComplete) {
                onComplete(message.data);
            }
        }
        // ...
    }
}, [onComplete, onError, loadDocumentsList, t]);
```

**效果**:
- ✅ 文档处理完成后，1秒后自动从待处理列表中消失
- ✅ 列表只显示待处理和处理中的文档
- ✅ 已完成的文档不再占用视图空间

---

### 3. 前端 - 浏览器视图添加按钮

**文件**: `UI/src/components/document/DocumentBrowser.jsx`

#### 修改点1: 添加按钮到操作栏

```javascript
<Tooltip title={t('document.browse.addToFlowView')}>
  <Button
    type="text"
    size="small"
    icon={<SyncOutlined />}
    onClick={(e) => {
      e.stopPropagation()
      handleAddToFlowView(record)
    }}
  />
</Tooltip>
```

#### 修改点2: 实现处理函数

```javascript
/**
 * 加入流程视图 / Add to flow view
 * 
 * 将文档从存储目录复制到临时目录，并跳转到流程视图
 */
const handleAddToFlowView = useCallback(async (item) => {
  try {
    antdMessage.loading({ content: t('document.browse.addingToFlow'), key: 'addToFlow' })
    
    // 调用后端API：从storage读取并写入到documents临时目录
    const response = await axios.post('/api/documents/copy-to-pending', {
      path: item.path,
      fileName: item.name
    })

    if (response.data && response.data.success) {
      antdMessage.success({ 
        content: t('document.browse.addToFlowSuccess'), 
        key: 'addToFlow' 
      })
      
      // 跳转到流程视图，并传递文档ID
      const documentId = response.data.documentId || item.name
      window.location.hash = `#/documents?view=flow&docId=${encodeURIComponent(documentId)}`
    } else {
      antdMessage.error({ 
        content: response.data.message || t('document.browse.addToFlowFailed'), 
        key: 'addToFlow' 
      })
    }
  } catch (error) {
    console.error('Failed to add to flow view:', error)
    antdMessage.error({ 
      content: t('document.browse.addToFlowFailed'), 
      key: 'addToFlow' 
    })
  }
}, [t])
```

**效果**:
- ✅ 文件列表每个文件都有"加入流程视图"按钮（SyncOutlined图标）
- ✅ 点击按钮后显示加载提示
- ✅ 成功后自动跳转到流程视图并选中该文档
- ✅ 失败时显示错误信息

---

### 4. 后端 - API端点实现

**文件**: `omni-agent-web/.../DocumentManagementController.java`

#### API定义

```java
/**
 * 将文档从存储目录复制到待处理目录（加入流程视图）⭐
 * POST /api/documents/copy-to-pending
 *
 * 用于将已完成的文档重新加入流程视图进行处理
 * 使用虚拟路径系统：从 storage 读取，写入到 data/documents 临时目录
 */
@PostMapping("/copy-to-pending")
public Map<String, Object> copyToPending(@RequestBody Map<String, String> request) {
    // ...
}
```

#### 实现逻辑

```java
// 1. 从存储服务读取文档（虚拟路径系统）⭐
log.debug("从存储服务读取文档: {}", virtualPath);
byte[] fileData = storageService.readFile(virtualPath);

// 2. 确保监听目录存在
Path watchDir = Paths.get(watchDirectory);
if (!Files.exists(watchDir)) {
    Files.createDirectories(watchDir);
}

// 3. 写入到监听目录（临时目录）
Path targetFile = watchDir.resolve(fileName);

// 如果文件已存在，生成新文件名避免覆盖
if (Files.exists(targetFile)) {
    // 生成新名称: 文件名_1.pdf, 文件名_2.pdf ...
}

Files.write(targetFile, fileData);

// 4. 从存储服务中删除原文档 ⭐ (避免索引冲突)
try {
    boolean deleted = storageService.deleteFile(virtualPath);
    if (deleted) {
        log.info("🗑️ 已从存储服务删除原文档: {}", virtualPath);
    }
} catch (Exception deleteEx) {
    log.warn("⚠️ 删除原文档时出错（继续处理）: {}", virtualPath, deleteEx);
    // 不影响主流程，继续处理
}

// 5. 返回文档ID供前端使用
result.put("documentId", documentId);
result.put("fileName", fileName);
```

**关键特性**:
- ✅ 使用虚拟路径系统，支持多种存储后端（File/MinIO/S3/MongoDB/Elasticsearch）
- ✅ 不是"复制"操作，而是从storage读取后写入临时目录
- ✅ 自动处理文件名冲突（生成_1、_2等后缀）
- ✅ 从storage删除原文档，避免重复索引冲突 ⭐
- ✅ 详细的日志记录便于调试

---

### 5. 国际化支持

#### 中文 (`UI/src/lang/zh.js`)

```javascript
browse: {
  // ... 其他翻译 ...
  
  // 流程视图 / Flow View
  addToFlowView: '加入流程视图',
  addingToFlow: '正在加入流程视图...',
  addToFlowSuccess: '已加入流程视图，即将跳转',
  addToFlowFailed: '加入流程视图失败',
}
```

#### 英文 (`UI/src/lang/en.js`)

```javascript
browse: {
  // ... other translations ...
  
  // Flow View
  addToFlowView: 'Add to Flow View',
  addingToFlow: 'Adding to flow view...',
  addToFlowSuccess: 'Added to flow view, redirecting...',
  addToFlowFailed: 'Failed to add to flow view',
}
```

---

## 📊 使用场景

### 场景1: 正常文档处理

```
1. 用户上传文档 → 文档出现在流程视图
2. 系统自动处理 → 显示处理进度
3. 处理完成 → 文档自动从列表消失 ⭐
4. 切换到浏览器视图 → 可以查看已完成的文档
```

### 场景2: 重新处理文档

```
1. 在浏览器视图找到文档
2. 点击"加入流程视图"按钮 ⭐
3. 自动跳转到流程视图
4. 文档出现在待处理列表
5. 可以重新配置策略并处理
```

### 场景3: 测试新策略

```
1. 已完成的文档存储在 storage
2. 想测试新的分块策略
3. 将文档重新加入流程视图
4. 配置新的分块参数
5. 开始处理并查看效果
```

---

## 🎨 用户体验

### 流程视图

**之前**:
- ❌ 已完成的文档一直显示在列表中
- ❌ 列表越来越长，难以管理
- ❌ 无法区分待处理和已完成的文档

**之后**:
- ✅ 只显示待处理和处理中的文档
- ✅ 完成后自动消失，列表保持清爽
- ✅ 清晰的状态区分

### 浏览器视图

**新增功能**:
- ✅ 每个文件都有"加入流程视图"按钮
- ✅ 一键将已完成文档重新加入处理队列
- ✅ 自动跳转，无缝体验

---

## 🔍 技术亮点

### 1. 虚拟路径系统

不依赖具体的存储实现，支持：
- ✅ 本地文件系统 (File)
- ✅ MinIO 对象存储
- ✅ AWS S3
- ✅ MongoDB GridFS
- ✅ Elasticsearch

### 2. 自动刷新机制

```javascript
// WebSocket监听处理完成事件
if (message.data.status === 'COMPLETED') {
    // 延迟刷新确保后端状态已更新
    setTimeout(() => {
        loadDocumentsList();
    }, 1000);
}
```

### 3. 智能文件名处理

```java
// 自动处理文件名冲突
if (Files.exists(targetFile)) {
    fileName = baseName + "_" + counter + extension;
}
```

---

## 📝 配置说明

### 监听目录配置

在 `application.yml` 中配置：

```yaml
omni-agent:
  file-watcher:
    watch-directory: ./data/documents  # 临时处理目录
```

### 存储目录

文档完成后自动保存到：
```
data/storage/documents/{documentId}/{filename}
```

---

## 🧪 测试验证

### 前提条件

⚠️ **重要**: 在测试前需要重新编译并启动后端服务：

```bash
# 1. 停止当前运行的服务
# 2. 重新编译
mvn clean install -DskipTests

# 3. 启动服务
cd omni-agent-web
mvn spring-boot:run
```

或者在 IDE 中重新启动 Spring Boot 应用。

### 测试步骤

1. **上传文档测试**
   ```
   1. 上传一个PDF文档
   2. 切换到流程视图
   3. 观察文档在待处理列表中
   4. 等待处理完成
   5. 验证文档自动消失 ✅
   ```

2. **重新加入测试**
   ```
   1. 切换到浏览器视图
   2. 找到已完成的文档
   3. 点击"加入流程视图"按钮
   4. 验证自动跳转 ✅
   5. 验证文档出现在待处理列表 ✅
   ```

3. **并发处理测试**
   ```
   1. 批量上传多个文档
   2. 观察它们依次处理
   3. 完成的文档逐个消失 ✅
   ```

---

## 🎉 总结

**新增功能**:
1. ✅ 处理完成后自动从流程视图移除
2. ✅ 浏览器视图可以重新加入流程
3. ✅ 基于虚拟路径的存储抽象
4. ✅ 完整的国际化支持

**用户价值**:
- ✅ 更清爽的流程视图界面
- ✅ 灵活的文档生命周期管理
- ✅ 支持重新处理已完成文档
- ✅ 无缝的操作体验

**技术价值**:
- ✅ 解耦存储实现
- ✅ 自动化状态管理
- ✅ 完善的错误处理
- ✅ 详细的日志记录

---

## 🔧 故障排除

### 问题1: 405 Method Not Allowed

**症状**:
```
Failed to load resource: the server responded with a status of 405 (Method Not Allowed)
POST http://localhost:8080/api/documents/copy-to-pending
```

**原因**: 后端代码已修改，但服务器没有重启，新的 API 端点未生效。

**解决方案**:
1. **停止当前运行的后端服务**
2. **重新编译项目**:
   ```bash
   mvn clean install -DskipTests
   ```
3. **重新启动后端**:
   ```bash
   cd omni-agent-web
   mvn spring-boot:run
   ```
   或在 IDE 中重启 Spring Boot 应用

### 问题2: Ant Design Message 警告

**症状**:
```
Warning: [antd: message] Static function can not consume context like dynamic theme. 
Please use 'App' component instead.
```

**原因**: 使用了 `antd` 的静态 `message` 方法而不是 `App` 组件的 message。

**解决方案**: ✅ 已修复
- 使用 `useMessage()` hook 代替静态方法
- 移除 `import { message as antdMessage } from 'antd'`
- 所有 `antdMessage.xxx()` 改为 `message.xxx()`

### 问题3: 文档未从流程视图消失

**症状**: 文档处理完成后仍然显示在待处理列表中。

**可能原因**:
1. WebSocket 连接未建立
2. 后端状态更新延迟
3. 前端刷新间隔太短

**解决方案**:
- 检查浏览器控制台的 WebSocket 连接状态
- 等待 1-2 秒后手动点击"刷新"按钮
- 切换到其他视图再切回流程视图

### 问题4: 无法读取文档数据

**症状**:
```json
{
  "success": false,
  "message": "无法读取文档数据（文件可能不存在或为空）"
}
```

**可能原因**:
1. 虚拟路径不正确
2. 存储服务未正确配置
3. 文档确实不存在

**解决方案**:
- 检查浏览器控制台，确认传递的 `path` 参数
- 在后端日志中查看完整的虚拟路径
- 确认存储服务配置正确（File/MinIO/S3/MongoDB 等）

---

## 🚀 后续优化方向

1. **批量操作** - 支持批量重新加入流程视图
2. **历史记录** - 记录文档的处理历史
3. **版本对比** - 对比不同策略的处理结果
4. **定时清理** - 自动清理临时目录中的过期文件

---

**实现完成时间**: 2025年12月26日  
**开发人员**: GitHub Copilot  
**测试状态**: 待验证 ⏳

