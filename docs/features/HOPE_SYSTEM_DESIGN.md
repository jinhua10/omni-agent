# HOPE分层知识管理系统设计文档

**系统名称：** HOPE (Hierarchical Omni-Agent Persistent Engine)  
**版本：** 1.0.0  
**文档创建时间：** 2025-12-31  
**状态：** ✅ 已实现（但缺少文档）

---

## 📋 目录

1. [系统概述](#系统概述)
2. [核心概念](#核心概念)
3. [架构设计](#架构设计)
4. [核心组件](#核心组件)
5. [使用指南](#使用指南)
6. [配置说明](#配置说明)
7. [API参考](#api参考)

---

## 🎯 系统概述

### 什么是HOPE系统？

HOPE (Hierarchical Omni-Agent Persistent Engine) 是 OmniAgent 的**分层知识管理系统**，通过将知识分为三个层级，实现智能的知识检索和问答。

### 为什么需要HOPE？

在传统的RAG系统中，所有知识都被平等对待，这导致：
- ❌ 频繁访问的知识检索效率低
- ❌ 核心知识和临时知识混在一起
- ❌ 无法根据知识的重要性优化查询策略

HOPE通过**分层管理**解决这些问题：
- ✅ 高频知识快速响应
- ✅ 核心知识优先级高
- ✅ 智能路由到合适的知识层

### 核心价值

1. **提升检索效率** - 根据问题类型选择合适的知识层
2. **优化知识组织** - 按重要性和访问频率分层
3. **支持知识演化** - 根据访问模式动态调整层级
4. **统计和分析** - 跟踪每层的使用情况

---

## 🏗️ 核心概念

### 三层知识结构

HOPE系统将知识分为三个层级：

#### 1. Permanent Layer (持久层) ⭐⭐⭐

**定位：** 长期稳定的核心知识

**特点：**
- 📌 不经常变化
- 🎯 高优先级
- 📚 系统级知识

**适用场景：**
- 系统配置和设置
- 基础概念和定义
- 核心业务规则
- 架构设计文档

**示例问题：**
- "如何配置RAG系统？"
- "什么是知识域？"
- "系统支持哪些文档格式？"

#### 2. Ordinary Layer (普通层) ⭐⭐

**定位：** 一般性知识

**特点：**
- 📖 中等优先级
- 🔄 可能变化
- 💼 业务知识

**适用场景：**
- 业务流程
- 领域知识
- 项目文档
- 用户手册

**示例问题：**
- "如何处理PDF文档？"
- "文档分块的策略是什么？"
- "RAG索引如何构建？"

#### 3. High Frequency Layer (高频层) ⭐

**定位：** 频繁访问的知识

**特点：**
- ⚡ 快速响应
- 🔥 热点问题
- 📊 基于统计

**适用场景：**
- 常见问题（FAQ）
- 热点话题
- 近期活跃的知识
- 用户反馈多的内容

**示例问题：**
- "如何快速上手？"
- "常见错误有哪些？"
- "最新功能介绍"

### 知识流转

```
┌─────────────────────────────────────────────────────┐
│  用户问题                                             │
└─────────────────────────────────────────────────────┘
                        ↓
┌─────────────────────────────────────────────────────┐
│  QuestionClassifier (问题分类器)                      │
│  - 基于关键词                                         │
│  - 基于正则表达式                                      │
│  - 基于自定义规则                                      │
└─────────────────────────────────────────────────────┘
                        ↓
        ┌───────────────┼───────────────┐
        ↓               ↓               ↓
  ┌──────────┐   ┌──────────┐   ┌──────────┐
  │持久层     │   │普通层     │   │高频层     │
  │Permanent │   │Ordinary  │   │High Freq │
  └──────────┘   └──────────┘   └──────────┘
        ↓               ↓               ↓
┌─────────────────────────────────────────────────────┐
│  RAG Service (语义搜索)                               │
└─────────────────────────────────────────────────────┘
                        ↓
┌─────────────────────────────────────────────────────┐
│  查询结果 + 置信度                                     │
└─────────────────────────────────────────────────────┘
```

---

## 🏛️ 架构设计

### 系统架构

```
┌─────────────────────────────────────────────────────┐
│  应用层 (Application Layer)                          │
│  - Web API                                          │
│  - Service                                          │
└─────────────────────────────────────────────────────┘
                        ↓
┌─────────────────────────────────────────────────────┐
│  HOPE核心层 (HOPE Core Layer)                        │
│  ┌─────────────────────────────────────────────┐   │
│  │  HOPEKnowledgeManager                       │   │
│  │  - 查询协调                                   │   │
│  │  - 统计分析                                   │   │
│  │  - 层级管理                                   │   │
│  └─────────────────────────────────────────────┘   │
│  ┌─────────────────────────────────────────────┐   │
│  │  QuestionClassifier                         │   │
│  │  - 问题分类                                   │   │
│  │  - 规则匹配                                   │   │
│  │  - 层级建议                                   │   │
│  └─────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────┘
                        ↓
┌─────────────────────────────────────────────────────┐
│  持久化层 (Persistence Layer)                        │
│  ┌─────────────────────────────────────────────┐   │
│  │  HopePersistence (接口)                     │   │
│  │  - InMemoryHopePersistence (内存实现)       │   │
│  │  - KnowledgeRegistryHopePersistence (生产)  │   │
│  └─────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────┘
                        ↓
┌─────────────────────────────────────────────────────┐
│  RAG服务层 (RAG Service Layer)                       │
│  - 语义搜索                                          │
│  - 向量检索                                          │
│  - 文档索引                                          │
└─────────────────────────────────────────────────────┘
```

### 模块组成

| 模块 | 包路径 | 职责 |
|------|--------|------|
| HOPEKnowledgeManager | top.yumbo.ai.omni.core.hope | 核心协调器 |
| QuestionClassifier | top.yumbo.ai.omni.core.hope | 问题分类器 |
| HopePersistence | top.yumbo.ai.omni.core.hope.persistence | 持久化接口 |
| InMemoryHopePersistence | ...persistence.impl | 内存实现 |
| KnowledgeRegistryHopePersistence | ...persistence.impl | 知识注册表实现 |
| HopePersistenceAutoConfiguration | ...hope.config | 自动配置 |

---

## 🔧 核心组件

### 1. HOPEKnowledgeManager

**职责：** HOPE系统的核心协调器

#### 主要方法

```java
@Service
public class HOPEKnowledgeManager {
    
    /**
     * 查询知识
     * 
     * @param question 用户问题
     * @param maxResults 最大结果数
     * @return 查询结果
     */
    public QueryResult query(String question, int maxResults);
    
    /**
     * 智能查询（增强版，支持上下文）
     * 
     * @param question 用户问题
     * @param context 上下文信息
     * @return 查询结果
     */
    public QueryResult smartQuery(String question, String context);
    
    /**
     * 获取层级统计信息
     */
    public Map<String, LayerStats> getLayerStats();
    
    /**
     * 重置统计信息
     */
    public void resetStats();
}
```

#### 查询流程

```java
public QueryResult query(String question, int maxResults) {
    // 1. 分类问题
    String questionType = questionClassifier.classify(question);
    String suggestedLayer = questionClassifier.getSuggestedLayer(questionType);
    
    // 2. 使用 RAG 进行语义搜索
    List<Document> documents = ragService.semanticSearch(question, maxResults);
    
    // 3. 更新统计信息
    updateStats(suggestedLayer);
    
    // 4. 构建结果（包含置信度计算）
    return buildResult(question, questionType, suggestedLayer, documents);
}
```

#### QueryResult 模型

```java
public static class QueryResult {
    private String question;             // 用户问题
    private String questionType;         // 问题类型
    private String suggestedLayer;       // 建议的层级
    private List<Document> documents;    // 检索到的文档
    private Long queryTimeMs;            // 查询耗时
    private Boolean success;             // 是否成功
    private Double confidence;           // 置信度
    private String answer;               // 学习系统的答案（可选）
}
```

### 2. QuestionClassifier

**职责：** 决定使用哪一层知识回答问题

#### 主要方法

```java
@Component
public class QuestionClassifier {
    
    /**
     * 分类问题
     */
    public String classify(String question);
    
    /**
     * 获取建议的层级
     */
    public String getSuggestedLayer(String questionType);
    
    /**
     * 获取所有问题类型
     */
    public List<QuestionTypeConfig> getAllQuestionTypes();
    
    /**
     * 添加问题类型
     */
    public void addQuestionType(QuestionTypeConfig config);
    
    /**
     * 更新问题类型
     */
    public void updateQuestionType(QuestionTypeConfig config);
    
    /**
     * 删除问题类型
     */
    public void deleteQuestionType(String typeName);
}
```

#### 分类逻辑

1. **关键词匹配** - 检查问题中是否包含预定义关键词
2. **正则表达式匹配** - 使用正则表达式匹配问题模式
3. **默认分类** - 如果都不匹配，使用默认类型

#### QuestionTypeConfig 模型

```java
@Data
@Builder
public class QuestionTypeConfig {
    private String typeName;              // 类型名称
    private String layer;                 // 层级 (permanent/ordinary/high_frequency)
    private List<String> keywords;        // 关键词列表
    private List<String> patterns;        // 正则表达式列表
    private String description;           // 描述
}
```

### 3. HopePersistence

**职责：** 持久化问题类型配置

#### 接口定义

```java
public interface HopePersistence {
    
    /**
     * 保存问题类型配置
     */
    void save(QuestionTypeConfig config);
    
    /**
     * 查找问题类型配置
     */
    Optional<QuestionTypeConfig> findByTypeName(String typeName);
    
    /**
     * 获取所有配置
     */
    List<QuestionTypeConfig> findAll();
    
    /**
     * 删除配置
     */
    void delete(String typeName);
}
```

#### 实现类

##### InMemoryHopePersistence

**用途：** 开发和测试

**特点：**
- 基于ConcurrentHashMap
- 无持久化
- 重启丢失

##### KnowledgeRegistryHopePersistence

**用途：** 生产环境

**特点：**
- 基于KnowledgeRegistry
- 持久化到文件/数据库
- 支持多种后端

---

## 📚 使用指南

### 快速开始

#### 1. 添加依赖

```xml
<dependency>
    <groupId>top.yumbo.ai.omni</groupId>
    <artifactId>omni-agent-core</artifactId>
    <version>1.0.0</version>
</dependency>
```

#### 2. 基本使用

```java
@Autowired
private HOPEKnowledgeManager hopeManager;

// 查询知识
QueryResult result = hopeManager.query("如何配置RAG系统？", 5);

if (result.getSuccess()) {
    System.out.println("问题类型: " + result.getQuestionType());
    System.out.println("建议层级: " + result.getSuggestedLayer());
    System.out.println("置信度: " + result.getConfidence());
    System.out.println("找到文档数: " + result.getDocuments().size());
}
```

#### 3. 智能查询（带上下文）

```java
String context = "用户之前询问过文档处理相关的问题";
QueryResult result = hopeManager.smartQuery("如何分块？", context);
```

### 配置问题类型

#### 添加自定义问题类型

```java
@Autowired
private QuestionClassifier classifier;

QuestionTypeConfig systemConfig = QuestionTypeConfig.builder()
    .typeName("系统配置")
    .layer("permanent")
    .keywords(Arrays.asList("配置", "设置", "系统", "参数"))
    .patterns(Arrays.asList(
        "如何配置.*",
        "怎么设置.*",
        ".*配置.*"
    ))
    .description("系统配置相关问题")
    .build();

classifier.addQuestionType(systemConfig);
```

#### 预定义问题类型示例

```java
// 1. 系统配置 - Permanent Layer
QuestionTypeConfig.builder()
    .typeName("系统配置")
    .layer("permanent")
    .keywords(Arrays.asList("配置", "设置", "系统"))
    .build();

// 2. 文档处理 - Ordinary Layer
QuestionTypeConfig.builder()
    .typeName("文档处理")
    .layer("ordinary")
    .keywords(Arrays.asList("文档", "PDF", "上传", "处理"))
    .build();

// 3. 常见问题 - High Frequency Layer
QuestionTypeConfig.builder()
    .typeName("常见问题")
    .layer("high_frequency")
    .keywords(Arrays.asList("如何", "怎么", "快速", "简单"))
    .build();
```

### 查看统计信息

```java
Map<String, LayerStats> stats = hopeManager.getLayerStats();

for (Map.Entry<String, LayerStats> entry : stats.entrySet()) {
    LayerStats stat = entry.getValue();
    System.out.println("层级: " + entry.getKey());
    System.out.println("查询次数: " + stat.getQueryCount());
    System.out.println("平均耗时: " + stat.getAverageQueryTime() + "ms");
}
```

---

## ⚙️ 配置说明

### 持久化配置

#### 使用内存存储（默认）

```yaml
# application.yml
# 默认不需要配置，自动使用内存存储
```

#### 使用知识注册表存储

```java
@Configuration
public class HopeConfig {
    
    @Bean
    @ConditionalOnProperty(name = "hope.persistence.type", havingValue = "registry")
    public HopePersistence hopePersistence(KnowledgeRegistry registry) {
        return new KnowledgeRegistryHopePersistence(registry);
    }
}
```

```yaml
# application.yml
hope:
  persistence:
    type: registry
```

### 自动配置

HOPE系统通过Spring Boot自动配置启用：

```
META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports:
top.yumbo.ai.omni.hope.starter.config.HopePersistenceAutoConfiguration
```

**默认行为：**
- 如果没有配置`HopePersistence` Bean，自动创建`InMemoryHopePersistence`
- 自动注入`HOPEKnowledgeManager`和`QuestionClassifier`

---

## 📖 API参考

### HOPEKnowledgeManager API

| 方法 | 参数 | 返回值 | 说明 |
|------|------|--------|------|
| query | String question, int maxResults | QueryResult | 基础查询 |
| smartQuery | String question, String context | QueryResult | 智能查询（带上下文） |
| getLayerStats | - | Map<String, LayerStats> | 获取统计信息 |
| resetStats | - | void | 重置统计信息 |

### QuestionClassifier API

| 方法 | 参数 | 返回值 | 说明 |
|------|------|--------|------|
| classify | String question | String | 分类问题 |
| getSuggestedLayer | String questionType | String | 获取建议层级 |
| getAllQuestionTypes | - | List<QuestionTypeConfig> | 获取所有类型 |
| addQuestionType | QuestionTypeConfig | void | 添加类型 |
| updateQuestionType | QuestionTypeConfig | void | 更新类型 |
| deleteQuestionType | String typeName | void | 删除类型 |

### QueryResult 字段

| 字段 | 类型 | 说明 |
|------|------|------|
| question | String | 用户问题 |
| questionType | String | 问题类型 |
| suggestedLayer | String | 建议的层级 |
| documents | List<Document> | 检索到的文档 |
| queryTimeMs | Long | 查询耗时（毫秒） |
| success | Boolean | 是否成功 |
| confidence | Double | 置信度（0.0-1.0） |
| answer | String | 学习系统的答案（可选） |

### LayerStats 字段

| 字段 | 类型 | 说明 |
|------|------|------|
| layerName | String | 层级名称 |
| queryCount | Long | 查询次数 |
| totalQueryTime | Long | 总查询时间（毫秒） |
| averageQueryTime | Double | 平均查询时间（毫秒） |

---

## 🔍 高级话题

### 知识层级优化

未来可以基于以下因素动态调整知识层级：

1. **访问频率** - 高频访问的知识自动移到高频层
2. **时间衰减** - 长期未访问的知识降级
3. **重要性评分** - 基于用户反馈评分
4. **学习效果** - 基于问答效果调整

### 与学习系统集成

HOPE系统预留了学习系统接口：

```java
// QueryResult中的answer字段
private String answer; // 学习系统的答案
```

未来可以：
- 从学习系统获取已学习的答案
- 直接返回答案，无需RAG检索
- 提升响应速度

### 性能优化

1. **缓存策略** - 高频问题缓存结果
2. **并行查询** - 多层同时查询，合并结果
3. **索引优化** - 为每层创建独立索引

---

## 🐛 故障排查

### 常见问题

#### 1. 问题无法正确分类

**原因：** 缺少对应的问题类型配置

**解决：**
```java
// 添加更多问题类型配置
classifier.addQuestionType(customConfig);
```

#### 2. 查询结果置信度低

**原因：** 知识库中缺少相关知识

**解决：**
- 补充相关文档
- 检查RAG索引是否正确构建

#### 3. 统计信息不准确

**原因：** 统计未重置或持久化问题

**解决：**
```java
hopeManager.resetStats(); // 重置统计
```

---

## 📝 待办事项

### 功能增强

- [ ] 支持机器学习分类（替代规则匹配）
- [ ] 实现知识层级自动调整
- [ ] 支持多租户隔离
- [ ] 添加可视化统计界面

### 文档完善

- [ ] 添加更多使用示例
- [ ] 提供性能测试报告
- [ ] 编写最佳实践指南

---

## 📚 相关文档

- [模块索引文档](../../modules_readme.md)
- [第一批模块分析报告](../MODULE_ANALYSIS_BATCH_01.md)
- [知识网络架构设计](./KNOWLEDGE_NETWORK_ARCHITECTURE.md)
- [智能问答系统设计](./INTELLIGENT_QA_SYSTEM_DESIGN.md)

---

**文档版本：** 1.0.0  
**最后更新：** 2025-12-31  
**维护者：** OmniAgent Team

