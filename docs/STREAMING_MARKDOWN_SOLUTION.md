...
### 1. ✅ 批次级别的分区显示（已完成）
- ✅ 每个批次一个可折叠面板（Collapse 组件）
- ✅ 批次状态显示（等待中/处理中/已完成）
- ✅ 批次完成后自动展开下一批

**实现细节**：
- 后端发送 `BATCH_START` 和 `BATCH_END` 标记
- 前端用 Collapse 组件分批次展示
- 每个批次独立显示内容和状态
...
**实现细节**：
- 使用 `useEffect` 监听 `extractionResult` 变化
- `setTimeout` 实现 3 秒防抖
- 保存到 `/api/system/rag-config/document/{id}`
