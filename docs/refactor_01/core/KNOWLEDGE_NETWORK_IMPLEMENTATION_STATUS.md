...
详细分析：[Persistence 层架构分析](../PERSISTENCE_LAYER_ANALYSIS.md)

**问题：**
- `omni-agent-persistence-api` 及其 7 个 starter 模块几乎未使用
- 仅在 `HealthController` 中用于显示类名，无实际业务逻辑
- 功能与 `KnowledgeRegistry` 重叠
- 维护成本高，架构冗余

**建议：**
- ✅ 移除所有 persistence 模块（8个）
- ✅ 用 `KnowledgeRegistry` 替代（功能更强大）
- ✅ 简化架构，减少概念混淆
