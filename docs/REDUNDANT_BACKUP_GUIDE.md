# OmniAgent 冗余备份方案完整指南

## 📋 目录

1. [方案概述](#方案概述)
2. [冗余策略](#冗余策略)
3. [快速启动](#快速启动)
4. [配置详解](#配置详解)
5. [监控和管理](#监控和管理)
6. [API接口](#api接口)
7. [故障恢复](#故障恢复)
8. [最佳实践](#最佳实践)

---

## 方案概述

### 🎯 设计目标

- ✅ **数据安全**：多重备份确保数据不丢失
- ✅ **高可用性**：主备自动切换，服务不中断
- ✅ **异构冗余**：不同技术栈避免单点故障
- ✅ **自动监控**：实时健康检查和故障恢复
- ✅ **数据一致性**：定期校验和自动修复

### 🏗️ 架构图

```
┌─────────────────────────────────────────────────────────┐
│                   应用层 (Application)                    │
└─────────────────────────────────────────────────────────┘
                           ↓
┌─────────────────────────────────────────────────────────┐
│               冗余备份监控服务 (Monitoring)                │
│  • 健康检查  • 一致性验证  • 自动恢复  • 状态报告         │
└─────────────────────────────────────────────────────────┘
                           ↓
┌─────────────────────────────────────────────────────────┐
│                  持久化层 (Persistence)                    │
├─────────────────────────────────────────────────────────┤
│  主存储 (Primary)                                         │
│  └─ SQLite (轻量级、高性能)                               │
│                                                           │
│  备份存储 (Secondary) - 异构冗余                          │
│  ├─ File   (文件系统、人工可读)                           │
│  └─ H2     (嵌入式数据库、兼容性好)                       │
└─────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────┐
│                文档存储层 (Document Storage)                │
├─────────────────────────────────────────────────────────┤
│  主存储 (Primary)                                         │
│  └─ File System (./data/primary/documents/)              │
│                                                           │
│  备份存储 (Backup) - 同种冗余                             │
│  ├─ Backup1 (./data/backup/documents/backup1/)          │
│  ├─ Backup2 (./data/backup/documents/backup2/)          │
│  └─ Backup3 (./data/backup/documents/backup3/)          │
└─────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────┐
│                    RAG检索层 (RAG)                         │
├─────────────────────────────────────────────────────────┤
│  主检索 (Primary)                                         │
│  └─ Lucene (高性能全文检索)                               │
│                                                           │
│  备份检索 (Backup) - 异构冗余                             │
│  └─ SQLite FTS5 (备用全文检索)                           │
└─────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────┐
│                    AI服务层 (AI)                           │
├─────────────────────────────────────────────────────────┤
│  主服务 (Primary)                                         │
│  └─ Online API (千问/OpenAI/Claude)                      │
│                                                           │
│  故障转移 (Failover)                                      │
│  └─ Local Ollama (本地部署)                              │
└─────────────────────────────────────────────────────────┘
```

---

## 冗余策略

### 1️⃣ 持久化层 - 异构冗余

| 存储类型 | 角色 | 优势 | 用途 |
|---------|------|------|------|
| **SQLite** | 主存储 | 轻量、高性能、零配置 | 主要读写操作 |
| **File** | 备份1 | 人工可读、易于检查 | 故障排查、人工恢复 |
| **H2** | 备份2 | 兼容性好、标准SQL | 异构备份、迁移友好 |

**写入策略**：
- 主存储：同步写入（保证一致性）
- 备份存储：异步写入（提高性能）

### 2️⃣ 文档存储层 - 同种冗余

| 路径 | 角色 | 同步策略 |
|------|------|---------|
| `./data/primary/documents/` | 主存储 | 实时写入 |
| `./data/backup/documents/backup1/` | 备份1 | 5分钟同步 |
| `./data/backup/documents/backup2/` | 备份2 | 5分钟同步 |
| `./data/backup/documents/backup3/` | 备份3 | 5分钟同步 |

**特点**：
- 多路径物理隔离
- 定时自动同步
- 可配置到不同磁盘/挂载点

### 3️⃣ RAG检索层 - 异构冗余

| 引擎 | 角色 | 特点 |
|------|------|------|
| **Lucene** | 主检索 | 高性能、丰富功能 |
| **SQLite FTS5** | 备份检索 | 轻量、易维护 |

**自动同步**：
- 主索引更新时自动同步到备份
- 备份索引独立存储

### 4️⃣ AI服务层 - 故障转移

| 提供商 | 角色 | 优先级 |
|--------|------|--------|
| **Online API** | 主服务 | P0 |
| **Local Ollama** | 备用服务 | P1 |

**故障转移策略**：
- 在线API失败 → 自动切换到本地Ollama
- 支持手动指定优先级

---

## 快速启动

### 步骤1：编译项目

```bash
cd D:\Jetbrains\omni-agent
mvn clean install -DskipTests
```

### 步骤2：启动应用（使用冗余配置）

```bash
cd omni-agent-example-basic
mvn spring-boot:run -Dspring-boot.run.profiles=redundant
```

或者使用环境变量：

```bash
export SPRING_PROFILES_ACTIVE=redundant
mvn spring-boot:run
```

### 步骤3：验证启动

访问健康检查接口：

```bash
curl http://localhost:8080/api/backup/health
```

预期响应：

```json
{
  "success": true,
  "healthy": true,
  "totalBackends": 3,
  "healthyBackends": 3,
  "message": "All backup backends are healthy"
}
```

---

## 配置详解

### 核心配置文件

配置文件位置：`src/main/resources/application-redundant.yml`

### 1. 持久化配置

```yaml
omni-agent:
  persistence:
    strategy: composite          # 启用组合策略
    primary-type: sqlite         # 主存储类型
    secondary-types:             # 备份存储
      - file
      - h2
    
    sqlite:
      db-path: ./data/primary/sqlite/omni-agent.db
      auto-create-tables: true
    
    file:
      base-path: ./data/backup/persistence/file
      auto-backup: true
    
    h2:
      jdbc-url: jdbc:h2:file:./data/backup/persistence/h2/omni-agent
```

### 2. 文档存储配置

```yaml
omni-agent:
  document-storage:
    type: file
    file:
      base-path: ./data/primary/documents
      backup-paths:                    # 多路径备份
        - ./data/backup/documents/backup1
        - ./data/backup/documents/backup2
        - ./data/backup/documents/backup3
      auto-sync-backup: true           # 自动同步
      sync-interval: 300000            # 5分钟
```

### 3. 监控配置

```yaml
omni-agent:
  backup:
    monitoring:
      enabled: true
      check-interval: 60000          # 1分钟检查一次
    
    health-check:
      enabled: true
      alert-on-failure: true
    
    auto-recovery:
      enabled: true
      max-retry: 3
      retry-interval: 5000           # 5秒
    
    consistency-check:
      enabled: true
      check-interval: 3600000        # 1小时
      auto-repair: true
```

---

## 监控和管理

### 启动日志

成功启动时会看到：

```log
2025-12-16 20:40:00 [main] INFO  t.y.a.o.e.b.backup.BackupMonitoringService
  - 🔍 BackupMonitoringService initialized
  - Monitoring enabled: true
  - Health check enabled: true
  - Auto recovery enabled: true

2025-12-16 20:40:00 [main] INFO  t.y.a.o.e.b.backup.BackupMonitoringService
  - ✅ Registered backup backend: primary
  - ✅ Registered backup backend: secondary-file
  - ✅ Registered backup backend: secondary-h2
```

### 定时任务

系统会自动执行：

1. **健康检查**：每1分钟
2. **一致性检查**：每1小时
3. **故障恢复**：检测到故障时自动触发

---

## API接口

### 1. 获取备份状态

**请求**：

```bash
GET http://localhost:8080/api/backup/status
```

**响应**：

```json
{
  "success": true,
  "timestamp": "2025-12-16T20:40:00",
  "totalBackends": 3,
  "healthyBackends": 3,
  "unhealthyBackends": 0,
  "statistics": {
    "healthyChecks": 120,
    "unhealthyChecks": 0,
    "failedChecks": 0,
    "recoverySuccess": 0,
    "recoveryFailures": 0,
    "consistencyChecks": 2,
    "inconsistencyDetected": 0,
    "repairSuccess": 0,
    "repairFailures": 0
  }
}
```

### 2. 健康检查摘要

**请求**：

```bash
GET http://localhost:8080/api/backup/health
```

**响应**：

```json
{
  "success": true,
  "healthy": true,
  "totalBackends": 3,
  "healthyBackends": 3,
  "message": "All backup backends are healthy"
}
```

### 3. 手动触发健康检查

**请求**：

```bash
POST http://localhost:8080/api/backup/check
```

**响应**：

```json
{
  "success": true,
  "message": "Health check completed successfully"
}
```

### 4. 手动触发一致性检查

**请求**：

```bash
POST http://localhost:8080/api/backup/consistency-check
```

**响应**：

```json
{
  "success": true,
  "message": "Consistency check completed successfully"
}
```

### 5. 获取统计信息

**请求**：

```bash
GET http://localhost:8080/api/backup/statistics
```

**响应**：

```json
{
  "success": true,
  "healthyChecks": 120,
  "unhealthyChecks": 0,
  "failedChecks": 0,
  "recoverySuccess": 0,
  "recoveryFailures": 0,
  "consistencyChecks": 2,
  "inconsistencyDetected": 0,
  "repairSuccess": 0,
  "repairFailures": 0,
  "healthRate": "100.00%",
  "recoveryRate": "100.00%"
}
```

---

## 故障恢复

### 自动故障恢复流程

```
检测到故障 → 记录日志 → 触发恢复 → 重试3次 → 成功/失败
                ↓
            每次间隔5秒
```

### 故障恢复日志示例

```log
2025-12-16 20:45:00 [scheduler] WARN  t.y.a.o.e.b.backup.BackupMonitoringService
  - ⚠️  Backend secondary-file is unhealthy

2025-12-16 20:45:00 [scheduler] INFO  t.y.a.o.e.b.backup.BackupMonitoringService
  - 🔧 Attempting to recover backend: secondary-file

2025-12-16 20:45:05 [scheduler] INFO  t.y.a.o.e.b.backup.BackupMonitoringService
  - ✅ Backend secondary-file recovered successfully
```

### 一致性修复日志示例

```log
2025-12-16 21:00:00 [scheduler] WARN  t.y.a.o.e.b.backup.BackupMonitoringService
  - ⚠️  Data inconsistency detected in backend: secondary-file

2025-12-16 21:00:00 [scheduler] INFO  t.y.a.o.e.b.backup.BackupMonitoringService
  - 🔧 Repairing data inconsistency in backend: secondary-file

2025-12-16 21:00:01 [scheduler] INFO  t.y.a.o.e.b.backup.BackupMonitoringService
  - ✅ Data repaired successfully in backend: secondary-file
```

---

## 最佳实践

### 1. 存储路径规划

推荐将不同备份存储到不同物理磁盘：

```yaml
document-storage:
  file:
    base-path: /disk1/data/primary/documents     # 主磁盘
    backup-paths:
      - /disk2/backup/documents/backup1          # 备份磁盘2
      - /disk3/backup/documents/backup2          # 备份磁盘3
      - /nas/backup/documents/backup3            # NAS网络存储
```

### 2. 监控告警集成

建议集成到监控系统：

```java
@Component
public class BackupAlertService {
    
    @EventListener
    public void onBackupFailure(BackupFailureEvent event) {
        // 发送到企业微信/钉钉/邮件
        alertManager.send(event);
    }
}
```

### 3. 定期备份验证

建议每天验证一次备份完整性：

```bash
# 添加到 cron
0 2 * * * curl -X POST http://localhost:8080/api/backup/consistency-check
```

### 4. 备份数据导出

定期导出备份数据到外部存储：

```bash
# 压缩备份
tar -czf backup-$(date +%Y%m%d).tar.gz ./data/backup/

# 上传到云存储
aws s3 cp backup-$(date +%Y%m%d).tar.gz s3://my-bucket/
```

### 5. 容灾演练

定期进行故障演练：

1. 停止主存储
2. 验证系统切换到备份
3. 恢复主存储
4. 验证数据同步

---

## 常见问题

### Q1: 如何切换不同的冗余方案？

**A**: 修改 `application-redundant.yml`，调整 `primary-type` 和 `secondary-types`

### Q2: 备份是同步还是异步？

**A**: 主存储同步写入，备份存储异步写入（提高性能）

### Q3: 如何监控备份状态？

**A**: 访问 `/api/backup/status` 查看实时状态

### Q4: 故障恢复需要多长时间？

**A**: 默认重试3次，每次间隔5秒，最长15秒

### Q5: 能否动态添加备份？

**A**: 可以，修改配置后重启应用即可

---

## 总结

✅ **数据安全**：3重异构备份
✅ **高可用**：自动故障切换
✅ **自动化**：定时监控和修复
✅ **可观测**：完整的API和日志
✅ **灵活性**：可配置的冗余策略

祝您使用愉快！🚀

