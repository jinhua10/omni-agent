...
| # | 问题 | 影响 | 修复优先级 |
|---|------|------|-----------|
| 1 | **缺少国际化模块** | big_job.md规划的功能未实现 | 🔴 P0 |
| 2 | **缺少通用异常体系** | 错误处理不规范 | 🔴 P0 |
| 3 | **没有日志记录** | 生产环境排查困难 | 🔴 P0 |
| 4 | **setTimeout线程安全问题** | 并发场景bug | 🔴 P0 |

### 重要问题 (建议修复)

| # | 问题 | 影响 | 修复优先级 |
|---|------|------|-----------|
| 5 | **不支持异步调用** | 高并发性能受限 | 🟡 P1 |
| 6 | **不支持请求拦截器** | 功能不完整 | 🟡 P1 |
| 7 | **不支持文件上传/下载** | 功能不完整 | 🟡 P1 |
| 8 | **错误信息不够详细** | 调试困难 | 🟡 P1 |
...
**新增文件：**
```
omni-agent-common/src/main/java/top/yumbo/ai/omni/common/i18n/
├── MessageService.java          # 消息服务接口
├── MessageServiceImpl.java      # 默认实现
└── MessageCode.java             # 消息代码枚举
```
