package top.yumbo.ai.omni.knowledge.registry.service.rag;
...
public class RAGRebuildService {
    ...
    /**
     * 将 ChunkingService 的 Chunk 转换为 Storage 的 Chunk
     */
    private List<Chunk> convertChunkingChunksToStorageChunks(
            List<top.yumbo.ai.omni.chunking.Chunk> chunkingChunks,
            String documentId) {

        List<Chunk> storageChunks = new ArrayList<>();
        ...
        for (top.yumbo.ai.omni.chunking.Chunk chunkingChunk : chunkingChunks) {
            ...
            storageChunks.add(storageChunk);
        }
    ...
    private List<Document> rechunkAndConvert(String domainId) {
        if (chunkingService == null) {
            log.error("❌ ChunkingService 未配置，无法重新分块！");
            log.warn("💡 降级到使用已有分块");
            List<Chunk> allChunks = loadAllChunks();
        ...
        try {
            ...
            for (var entry : chunksByDoc.entrySet()) {
                ...
                // 使用 ChunkingService 重新分块 ⭐
                // 创建默认配置
                ChunkingConfig config = ChunkingConfig.builder().build();
                List<top.yumbo.ai.omni.chunking.Chunk> newChunks =
                        chunkingService.chunk(docId, fullContent, config);

                totalChunks += newChunks.size();

                // 转换 Chunk 类型并添加到结果
                List<Chunk> convertedChunks = convertChunkingChunksToStorageChunks(newChunks, docId);
                documents.addAll(convertChunksToDocuments(convertedChunks));
            }
